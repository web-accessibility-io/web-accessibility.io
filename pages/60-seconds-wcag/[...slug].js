...
export async function getStaticPaths({ locales, defaultLocale }) {
  const localesPost = locales
    .map((locale) => {
      ...
      const posts = getFiles('60-seconds-wcag', otherLocale);
...
export async function getStaticProps({ defaultLocale, locales, locale, params }) {
  const otherLocale = locale !== defaultLocale ? locale : '';
  const allPosts = await getAllFilesFrontMatter('60-seconds-wcag', otherLocale);
  const postIndex = allPosts.findIndex((post) => formatSlug(post.slug) === params.slug.join('/'));
  const prev = allPosts[postIndex + 1] || null;
  const next = allPosts[postIndex - 1] || null;
  const post = await getFileBySlug('60-seconds-wcag', params.slug.join('/'), otherLocale);
  ...
  fs.writeFileSync(
    `./public/60-seconds-wcag-feed${otherLocale === '' ? '' : `.${otherLocale}`}.xml`,
    rss
  );
  ...
  await locales.forEach(async (ilocal) => {
    ...
    const iAllPosts = await getAllFilesFrontMatter('60-seconds-wcag', otherLocale);
    iAllPosts.map((ipost) => {
...
export default function SixtySecWcag({ post, authorDetails, prev, next, availableLocales }) {
