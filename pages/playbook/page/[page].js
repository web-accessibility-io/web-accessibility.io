...
import { POSTS_PER_PAGE } from '../../playbook';
  ...
  const paths = (
    await Promise.all(
      locales.map(async (locale) => {
        ...
        const totalPosts = await getAllFilesFrontMatter('playbook', otherLocale); // don't forget to useotherLocale
...
export async function getStaticProps(context) {
  ...
  const otherLocale = locale !== defaultLocale ? locale : '';
  const posts = await getAllFilesFrontMatter('playbook', otherLocale);
