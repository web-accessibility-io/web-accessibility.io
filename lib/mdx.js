...
export function getFiles(type, otherLocale = '') {
  console.log('getFiles', type, otherLocale);
  const prefixPaths = path.join(root, 'data', type);
  const files =
    otherLocale === ''
      ? getAllFilesRecursively(prefixPaths).filter((path) => path.includes(`.d.md`))
...
export function formatSlug(slug) {
  ...
  // take the main root of slug e.g. post-name in post-name.d.mdx
  return slug.split('.')[0];
}
...
export async function getFileBySlug(type, slug, otherLocale = '') {
  const [mdxPath, mdPath] =
    otherLocale === ''
      ? [
          path.join(root, 'data', type, `${slug}.d.mdx`),
          path.join(root, 'data', type, `${slug}.d.md`),
        ]
      : [
